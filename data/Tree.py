...
def make_vocabulary(opt, name, filename, size):
    "Construct the word and feature vocabs."
    vocab = Dictionary(
        [PAD_WORD, UNK_WORD, BOS_WORD, EOS_WORD], lower=opt.lower)
    ...
    if size != 0:
        ...
        print("size after prune",vocab.size)
    else:
...
def traverse_python_tree(atok, root):
    ...
    for node in asttokens.util.walk(root):
        ...
        node_json["%s%s" % (NODE_FIX, current_idx)] = {"node": type(
            node).__name__, "children": [], "parent": None}
        # idx_upper = len(node_json)
        if not next(iter_children(node), None) is None:
            child_idx = 0
            for child in iter_children(node):
                child_idx += 1
                node_json["%s%s" % (NODE_FIX, current_idx)]['children'].insert(
                    0, "%s%s" % (NODE_FIX, current_global[current_idx]-child_idx+1))
        else:  # leaf node
            node_json["%s%s" % (NODE_FIX, current_idx)
                      ]['children'].append(atok.get_text(node))
...
#             idx += 1
#             global_idx += 1
#             node_json['%s%s' % (NODE_FIX, current_idx)]['children'].append('%s%s'
#                                                                            % (NODE_FIX, global_idx))
#         for c in tree.getChildren():
#             cx += 1
#             node_json, global_idx = traverse_java_tree(
#                 c, node_json, cx, global_idx)
...
def split_tree(tree_json, idx_upper):
    ...
    for k, node in tree_json.items():
        if len(node['children']) > 2:
            tree_json_splited['%s%s' % (NODE_FIX, idx_upper + 1)] = {
                'node': 'Tmp', 'children': node['children'][1:], 'parent': k}  # idx_upper + 2
            for ch in node['children'][1:]:
                tree_json_splited[ch]['parent'] = '%s%s' % (
                    NODE_FIX, idx_upper + 1)
            tree_json_splited[k]['children'] = [tree_json_splited[k]['children'][0], '%s%s' % (
                NODE_FIX, idx_upper + 1)]  # idx_upper + 2

            idx_upper += 1
    for k, node in tree_json_splited.items():
        children_length = len(
            [c for c in node['children'] if c.startswith(NODE_FIX)])
    ...
    for k, node in tree_json.items():
        ...
        if children_length == 1:
            ...
            for ch in tree_json[del_key]['children']:
                if ch.startswith(NODE_FIX) and ch in tree_json:
...
def json2tree_binary(tree_json, tree, idx, prev=None):
    if prev == None:
        tree.parent = prev
        tree.idx = idx
    children = [c for c in tree_json[idx]
                ['children'] if c.startswith(NODE_FIX)]
    if len(children) == 2:
        for c in children:
            t = Tree()
            t.parent = prev
