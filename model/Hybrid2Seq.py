...
class Hybrid2Seq(tf.keras.Model):
    ...
    def initialize(self, batch):
        ...
        for tree in trees:
            tree_encoder_context, tree_encoder_hidden = self.code_encoder(tree, lengths)
        ...
        tree_encoder_context_padded = tf.concat(tree_encoder_context_padded, axis=0)
        tree_encoder_hidden = (tf.concat(tree_encoder_hidden_0, axis=0), tf.concat(tree_encoder_hidden_1, axis=0))

        text_encoder_hidden, text_encoder_context = self.text_encoder(source_text)
        text_encoder_hidden = (tf.expand_dims(text_encoder_hidden[0], axis=1), tf.expand_dims(text_encoder_hidden[1], axis=1))
        ...
        return target, (embedding, initial_output, tree_encoder_hidden, tf.transpose(tree_encoder_context_padded, perm=[1, 0, 2]), text_encoder_hidden, tf.transpose(text_encoder_context, perm=[1, 0, 2]))

    def call(self, batch, training=False):
        targets, initial_states = self.initialize(batch)
