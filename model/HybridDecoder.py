...
class HybridDecoder(tf.keras.layers.Layer):
    def __init__(self, target_vocab_size: int, hidden_state_size: int = 512, num_layers: int = 1, dropout: float = 0.3, use_attention: bool = True) -> None:
        ...
        self.word_lut = tf.keras.layers.Embedding(
            target_vocab_size, hidden_state_size)
    ...
    def step(self, embedding, output, tree_hidden: tuple, tree_context, text_hidden: tuple, text_context):
        ...
        # tree o/p shape  (batch_size, word_embedding)
        # text o/p shape  (batch_size, word_embedding)
        # tree hidden shape  2
        # text hidden shape  2

        if self.use_attention:
            output, tree_attention, text_attention = self.attention(
                tree_output, tree_context, text_output, text_context)
        else:
            output = self.attention(
                tf.concat([tree_output, text_output], axis=-1))
