...
class GraphSequenceLayer(tf.keras.layers.Layer):
    ...
    def call(self, node_features: List[List[int]], adjacency_lists: Tuple[List[List[int]]], node_to_graph_map: List[int], batch_size: int, sequence_length: int):
        ...
        annotations_input = tf.convert_to_tensor([[float(feature[0])] for feature in node_features])

        print("annotations_input", annotations_input)
        print(tf.convert_to_tensor(node_to_graph_map))
        print(batch_size)
        ...
        for _ in range(sequence_length):
            ...
            annotations_level_out = self.annotations_layer(layer_input)
            print('asdasd')
