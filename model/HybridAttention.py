...
class HybridAttention(tf.keras.layers.Layer):
    def __init__(self, hidden_state_size: int) -> None:
        ...
        self.linear_in = tf.keras.layers.Dense(
            hidden_state_size, use_bias=False)
    ...
    def call(self, tree_inputs: tf.Tensor, tree_context: tf.Tensor, text_inputs: tf.Tensor, text_context: tf.Tensor) -> tuple:
        ...
        text_context = tf.reshape(text_context, [
                                  text_context.shape[1], text_context.shape[0], text_context.shape[2]])
        ...
        tree_attention_3d = tf.reshape(
            tree_attention, (tree_attention.shape[0], 1, tree_attention.shape[1]))
        ...
        tree_combined_context = tf.concat(
            [tree_weighted_context, tree_inputs], axis=1)

        text_weighted_context = tf.squeeze(
            tf.matmul(text_attention_3d, text_context), axis=1)
        text_combined_context = tf.concat(
            [text_weighted_context, text_inputs], axis=1)
